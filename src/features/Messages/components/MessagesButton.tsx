...
interface MessageButtonProps extends React.ComponentPropsWithoutRef<"button"> {
  loading?: boolean;
}
...
const MessagesButton = forwardRef<HTMLButtonElement, MessageButtonProps>(
  function MessagesButtonComponent({ loading, disabled, ...rest }, ref) {
    ...
    return (
      <button
        {...{ ...rest, disabled: loading || disabled, ref }}
        className={classes}
      >
        {loading ? <MessagesButtonLoadingSpinner /> : <MessagesButtonIcon />}
      </button>
    );
  }
);

/** An svg icon for use with the MessageButton component */
...
/** An animated svg loading icon for use with the MessageButton component */
const MessagesButtonLoadingSpinner = () => (
  <svg
    aria-hidden="true"
    role="status"
